...
@app.cell(hide_code=True)
def _(decode_hdf5_bytes, h5py, pd):
    with h5py.File("home/data/dr20_boss_clusters.h5", "r") as f:
...
def _(hclu, np):
    ...
    hclu["Association Type"] = hclu["Type"].map({
        "o": "open cluster",
        "m": "moving group",
        "g": "globular cluster",
    ...
    display_cols = ['Association Name', 'Association Type', 'Number of Members with BOSS Spectrum', 'HR24 Distance (pc)', 'HR24 Age (Myr)']
...
def _(cluster_name_val, hclu):
    try:
        age50 = str(
            round(
                (10 ** hclu.loc[hclu.Name == cluster_name_val].logAge50.values[0]) / 1e6
            )
        )
        age50_16 = str(
            round(
                (
                    (
                        (
                            10
                            ** hclu.loc[hclu.Name == cluster_name_val].logAge50.values[
                                0
                            ]
                        )
                        / 1e6
                    ...
                    - (
                        (
                            10
                            ** hclu.loc[hclu.Name == cluster_name_val].logAge16.values[
                                0
                            ]
                        )
                        / 1e6
            ...
            round(
                (
                    (
                        (
                            10
                            ** hclu.loc[hclu.Name == cluster_name_val].logAge84.values[
                                0
                            ]
                        )
                        / 1e6
        ...
        age_str = rf"Age: ${age50}^{{+{age84_50}}}_{{-{age50_16}}}$ Myr"
    except:
        age_str = "NaN"

    try:
        dist50 = str(
            round((hclu.loc[hclu.Name == cluster_name_val].dist50.values[0]), 2)
        )
        dist50_16 = str(
            round(
                (
                    ((hclu.loc[hclu.Name == cluster_name_val].dist50.values[0]))
                    - ((hclu.loc[hclu.Name == cluster_name_val].dist16.values[0]))
                ),
                2,
            )
        )
        dist84_50 = str(
            round(
                (
                    ((hclu.loc[hclu.Name == cluster_name_val].dist84.values[0]))
                    - ((hclu.loc[hclu.Name == cluster_name_val].dist50.values[0]))
                ),
                2,
            )
        )
        dist_str = rf"Distance: ${dist50}^{{+{dist84_50}}}_{{-{dist50_16}}}$ pc"
...
@app.cell(hide_code=True)
def _(allstar_select, mo, smoothing, spec_cmap, spec_color, spec_ranges):
